...
const Ad = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`https://jumpinghorses-examproject-4.onrender.com/api/ads/${id}`)
      .then(response => {
        setAd(response.data);
  ...
  const geocodeCity = async (city: string) => {
    try {
      const response = await axios.get(`https://jumpinghorses-examproject-4.onrender.com/geocode?city=${encodeURIComponent(city)}`);
  ...
  return (
    ...
    <div className="ad-container">
      ...
      <p><strong>Date:</strong> {convertToLocalDate(ad.Date)}</p>
        ...
        {ad.Bild && (
              <img 
                src={`https://jumpinghorses-examproject-4.onrender.com/uploads/${ad.Bild}`}
                className="ad-image"

...
const Ad = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`https://jumpinghorses-examproject-3.onrender.com/api/ads/${id}`)
      .then(response => {
        setAd(response.data);
  ...
  const geocodeCity = async (city: string) => {
    try {
      const response = await axios.get(`https://jumpinghorses-examproject-3.onrender.com/geocode?city=${encodeURIComponent(city)}`);
